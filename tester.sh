...
test_two ()
{
    ...
    ("$2/bin/$1" 4 410 200 200 > /dev/null)&
    i=1
    error=0
    while [ $i -lt 180 ];do
        printf "\r[%d...]" $i
...
test_three ()
{
    ...
    ("$2/bin/$1" 5 800 200 200 > /dev/null)&
    i=1
    error=0
    while [ $i -lt 180 ];do
        printf "\r[%d...]" $i
...
test_four ()
{
    ("$2/bin/$1" 4 410 200 200 $3 > "./log_$1")&
    sleep 10
    pgrep $1 > /dev/null
    if [ "$?" -eq 1 ];then
...
test_six ()
{
    ("$2/bin/$1" 10 410 200 200 > /dev/null)&
    sleep 2
    forks=$(pgrep $1 | wc -l)
    if [ "$forks" -eq 11 ];then
